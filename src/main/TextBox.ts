...
export default class TextBox extends Element {
  ...
  public static chopTextToFitWidth(text: string, font_size: number, width: number, word_wrap?: boolean): string[] {
    ...
    if (!Number.isFinite(width / font_size) || (width / font_size) < 5) {
      throw new Error(`innerwidth (${width}) / font size (${font_size}) must be >= 5`);
    }
  ...
  public estimateWidth(): number {
    return TextBox.estimateTextWidth(this.text, this.getStyleSet().getFontSize());
  }
  ...
  public getMarkup(): string {
    ...
    for (let i = 0; i < text_parts.length; i += 1) {
      ...
      const elem = new Text(this.getStyleSet(),
        ...
        this.getY() + this.padding[0] - (height / 2) + (this.getStyleSet().getLineHeight() * (i + 0.5)),
        text_parts[i]);
  ...
  private getTextParts(): string[] {
    return TextBox.chopTextToFitWidth(this.text,
