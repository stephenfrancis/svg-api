...
export default class Group extends Element {
  ...
  public addEllipse(
    ...
    const element = new Ellipse(
      this.current_styleset,
      x_pos,
      y_pos,
      x_rad,
      y_rad
  ...
  public addGroup(
    current_styleset?: StyleSet,
    x_pos?: number,
    y_pos?: number
  ): Group {
    const group = new Group(
      current_styleset || this.current_styleset,
      x_pos,
      y_pos
    );
  ...
  public addStraightLine(
    x_from: number,
    y_from: number,
    x_to: number,
    y_to: number
  ): StraightLine {
    const element = new StraightLine(
      this.current_styleset,
      x_from,
      y_from,
      x_to,
  ...
  public getExtremes(): Types.Extremes {
    ...
    this.elements.forEach((element: Element) => {
      try {
        ...
        extremes.x_min = Math.min(
          extremes.x_min,
          elem_extr.x_min + this.getX()
        );
        extremes.y_min = Math.min(
          extremes.y_min,
          elem_extr.y_min + this.getY()
        );
        extremes.x_max = Math.max(
          extremes.x_max,
          elem_extr.x_max + this.getX()
        );
        extremes.y_max = Math.max(
          extremes.y_max,
          elem_extr.y_max + this.getY()
        );
        // console.log(`checking extremes given elem: ${JSON.stringify(elem_extr)} to give: ${JSON.stringify(extremes)}`);
      } catch (e) {
        console.error(`error in getExtremes() for Element: ${element}`);
        throw e;
      }
    });
    return extremes;
  ...
  public getMarkup(): string {
    if (this.positioned) {
      return (
        ...
        "'>\n" +
        this.elements
          .map((element: Element) => element.getMarkup())
          .join("\n") +
        "\n</svg>"
      );
    }
